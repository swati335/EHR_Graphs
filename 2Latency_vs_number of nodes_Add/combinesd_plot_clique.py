...
selected_algo = 'QBFT'  # Example: only plot for Clique
...
# === DATA COLLECTION ===
latency_by_txn = defaultdict(dict)  # txn -> {node: latency}
    ...
    for file in os.listdir(algo_folder):
        if file.endswith(".csv") and "get" in file.lower():
            ...
            if "Latency(ms)" in df.columns:
                avg_latency = df["Latency(ms)"].max()
                latency_by_txn[txn][node] = avg_latency
...
for txn in transaction_counts:
    data = latency_by_txn[txn]
    nodes = sorted(data.keys())
    latencies = [data[n] for n in nodes]
    plt.plot(nodes, latencies, label=f"{txn} txns", color=colors[txn],linestyle='--',
             marker='o', linewidth=2.5, markersize=6, markeredgecolor='black')

plt.title(f"Max Latency vs Number of Nodes ({selected_algo})", fontsize=12, fontweight='bold')
